//
//  NoSigScriptUnlockViewController.swift
//  Bitcoin
//
...
class NoSigScriptUnlockViewController: UIViewController {
    ...
    // MARK: - No Signature Script Address Section
    private lazy var noSigAddressSectionLabel: UILabel = {
        let label = UILabel()
        label.text = "No-Sig Script Source Address"
        label.font = .systemFont(ofSize: 18, weight: .bold)
        label.textColor = .label
        return label
    }()
    
    private lazy var noSigAddressTextField: UITextField = {
        let textField = UITextField()
        textField.placeholder = "Enter no-sig script source address (P2WSH)"
    ...
    // MARK: - Hint Label
    private lazy var hintContainer: UIView = {
        let view = UIView()
    ...
    private lazy var hintLabel: UILabel = {
        let label = UILabel()
        label.text = "💡 Tip: No-sig script unlock does not require private key or signature, only preimage is needed. Witness stack structure: [secretHex, redeemScript]"
    ...
    private func setupUI() {
        title = "No-Sig Script: Unlock & Transfer"
        ...
        // No Signature Script Address
        contentView.addSubview(noSigAddressSectionLabel)
        contentView.addSubview(noSigAddressTextField)
        ...
        // Hint
        contentView.addSubview(hintContainer)
    ...
    private func setupConstraints() {
        ...
        // No Signature Script Address
        noSigAddressSectionLabel.snp.makeConstraints { make in
            make.top.equalTo(networkSegmentedControl.snp.bottom).offset(30)
            make.leading.trailing.equalToSuperview().inset(20)
        }
        
        noSigAddressTextField.snp.makeConstraints { make in
            make.top.equalTo(noSigAddressSectionLabel.snp.bottom).offset(15)
            make.leading.trailing.equalToSuperview().inset(20)
        ...
        redeemScriptSectionLabel.snp.makeConstraints { make in
            make.top.equalTo(noSigAddressTextField.snp.bottom).offset(30)
            make.leading.trailing.equalToSuperview().inset(20)
        }
        ...
        hintContainer.snp.makeConstraints { make in
            make.top.equalTo(lockHeightSecretStackView.snp.bottom).offset(20)
            make.leading.trailing.equalToSuperview().inset(20)
        }
        ...
        toAddressSectionLabel.snp.makeConstraints { make in
            make.top.equalTo(hintContainer.snp.bottom).offset(30)
            make.leading.trailing.equalToSuperview().inset(20)
        }
    ...
    @objc private func unlockButtonTapped() {
        // Validate input
        guard let noSigAddress = noSigAddressTextField.text?.trimmingCharacters(in: .whitespacesAndNewlines),
              !noSigAddress.isEmpty else {
            showAlert(title: "Error", message: "Please enter no-sig script source address")
            return
        }
    ...
    private func performUnlock(noSigAddress: String, toAddress: String, amountSats: Int64, feeSats: Int64, lockHeight: Int, secretHex: String, redeemScript: String, isTestnet: Bool) async {
        ...
        // Call unlock no-signature script address method using async/await
        let (success, result, errorMessage) = await bitcoin.unlockNoSigScriptAddress(
    ...
    @objc private func estimateFeeTapped() {
        ...
        guard let noSigAddress = noSigAddressTextField.text?.trimmingCharacters(in: .whitespacesAndNewlines),
              !noSigAddress.isEmpty else {
            showAlert(title: "Error", message: "Please enter no-sig script source address first")
            return
        }
        ...
        let inputsCount = 1
        let outputsCount = 1 // No-sig script unlock usually transfers full amount, so 1 output
